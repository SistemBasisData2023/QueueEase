    ...
    <div className="fixed inset-y-0 left-0 z-10 bg-base-200 w-1/2 md:w-64">
      ...
      <ul className="py-5">
        ...
        <li className="relative">
          <label htmlFor="settings-dropdown" className="btn btn-ghost m-1 text-xl font-medium capitalize" onClick={toggleDropdown}>
            Settings
          </label>
          <input type="checkbox" id="settings-dropdown" className="hidden" />
          <div className={`dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52 absolute ${isDropdownOpen ? 'block' : 'hidden'}`}>
